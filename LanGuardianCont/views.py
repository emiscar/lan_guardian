...
def contacto(request):
    ...
    if request.method=="POST":
        ...
        f = open("config2.txt", "a+")
        c = Connection(host="root" + "@" + "192.168.0.142", connect_kwargs={"password":"root"}, port=22).run('more config.txt')
        #c = Connection(host=usuario + "@" + ip, connect_kwargs={"password":clave}, port=puerto)
        #f.write(str(c.run('more config.txt')))
        msg = "{0.stdout}"
        print(msg.format(c))
        f.write(msg.format(c))
        f.close()
        print("Entre")
